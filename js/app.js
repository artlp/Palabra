...
function renderSettings() {
  setFormValue('sheet-url-input', appState.settings.sheetUrl || '');
  setFormValue('sheet-name-input', appState.settings.sheetName || '');
...
async function syncVocabulary({ silent = false } = {}) {
  ...
  try {
    const result = await loadWordsFromSource(appState.settings.sheetUrl, {
      sheetName: appState.settings.sheetName,
      fallbackUrl: './data/demo-words.csv',
    });
    appState.vocabulary = {
      words: result.words,
      sourceUrl: appState.settings.sheetUrl || '',
      sourceType: appState.settings.sheetUrl ? result.sourceType : 'Демо-словарь',
...
async function handleAuthChange(user) {
  ...
  if (!user) {
    ...
    if (!appState.vocabulary.words.length) await syncVocabulary({ silent: true });
    renderGlobal();
    return;
  }
  ...
  try {
    ...
    const sameGuestSource = (guestState.settings.sheetUrl || '') === (merged.settings.sheetUrl || '')
      && (guestState.settings.sheetName || '') === (merged.settings.sheetName || '');
    if (!merged.vocabulary.words.length && guestState.vocabulary.words.length && sameGuestSource) {
    ...
    if (!appState.vocabulary.words.length && guestState.vocabulary.words.length) appState.vocabulary = guestState.vocabulary;
    persistState();
...
async function handleSourceSettings(event) {
  event.preventDefault();
  appState.settings.sheetUrl = $('sheet-url-input').value.trim();
  appState.settings.sheetName = $('sheet-name-input').value.trim();
  persistState();
...
function bindEvents() {
  ...
  $('sync-button-top').addEventListener('click', () => syncVocabulary());
  $('dashboard-sync-button').addEventListener('click', () => syncVocabulary());
  $('words-sync-button').addEventListener('click', () => syncVocabulary());
